...
mono_sigctx_to_monoctx (void *sigctx, MonoContext *mctx)
{
	...
	memcpy (&mctx->regs, &context->X, sizeof (DWORD64) * 31);
	mctx->pc = context->Pc;
...
mono_monoctx_to_sigctx (MonoContext *mctx, void *sigctx)
{
	...
	memcpy (&context->X, &mctx->regs, sizeof (DWORD64) * 31);
	context->Pc = mctx->pc;
