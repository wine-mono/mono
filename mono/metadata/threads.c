...
mono_thread_detach_internal (MonoInternalThread *thread)
{
	...
	*/
	...
	if (!mono_g_hash_table_lookup_extended (threads, (gpointer)(gsize)thread->tid, NULL, (gpointer*) &value)) {
	...
	removed = mono_g_hash_table_remove (threads, (gpointer)(gsize)thread->tid);
	g_assert (removed);
...
wait_for_tids (struct wait_data *wait, guint32 timeout, gboolean check_state_change)
{
	...
	if (ret >= MONO_THREAD_INFO_WAIT_RET_SUCCESS_0 && ret < (MONO_THREAD_INFO_WAIT_RET_SUCCESS_0 + wait->num)) {
		...
		mono_threads_lock ();
		if (mono_g_hash_table_lookup (threads, (gpointer)(gsize) internal->tid) == internal)
