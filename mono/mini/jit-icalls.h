...
/*
 * This ARM backend does not implement the ARM hard-float procedure-call ABI:
 * ARM_FPU_VFP_HARD is rejected by mini-arm.h and ARM_NUM_REG_FPARGS is zero.
 * Generated helper calls therefore use the base AAPCS/core-register ABI for
 * floating-point values even when the C compiler targets armhf/VFP PCS.
 * Annotate only JIT helper entry points that expose float/double so those C
 * functions match the calls this backend emits.
 */
