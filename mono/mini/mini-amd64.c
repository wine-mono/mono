...
{
#ifdef TARGET_WIN32
	/* Wine Mono HACK: Go through the TIB.Self field for this. Most GS-relative
	 * addresses won't work correctly on macOS, but TIB.Self will. */
	x86_prefix (code, X86_GS_PREFIX);
	amd64_mov_reg_mem (code, dreg, 0x30, 8);
	if (tls_offset < 64) {
		amd64_mov_reg_membase (code, dreg, dreg, (tls_offset * 8) + 0x1480, 8);
	} else {
		guint8 *buf [16];

		g_assert (tls_offset < 0x440);
		/* Load TEB->TlsExpansionSlots */
		amd64_mov_reg_membase (code, dreg, dreg, 0x1780, 8);
